...
class CollisionBox(CollisionObject):
    ...
    def in_collision(self, target):
        ...
        for i in range(self.ndim):
            ...
            # print(target[i],self.location[i] + self.half_lengths[i],self.location[i] - self.half_lengths[i])
            if target[i] > ( self.location[i] + self.half_lengths[i] ) or target[i] < (self.location[i] - self.half_lengths[i]) :
                return False
...
class CollisionSphere(CollisionObject):
    ...
    def in_collision(self, target):
        ...
        if np.linalg.norm(self.location - target) <= self.radius:
            return True
